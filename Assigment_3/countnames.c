/**
 * Description: Counts the number of times each line appears in the file.  Input
 * can be a filename (first arg) or stdin. Names > 30 characters are truncated
 * with a warning.  Empty lines are skipped with a warning. Output is a series
 * of NameCountData structs written to stdout (pipe write end). Stderr is
 * redirected to PID.err, where PID is the current process id. Author names:
 * Steve Rubin, Raven Keszei Author emails: steven.rubin@sjsu.edu,
 * raven.keszei@sjsu.edu Last modified date: 3/24/26 Creation date: 2/11/26
 **/
...
void updateCount(char *name); // function prototype for the function that
                              // manages the linked list of names
...
int main(int argc, char *argv[]) {
  FILE *fp, *fpout;      // file point for input stream and pid.out file
  ...
  if (dup2(open(errFileName, O_WRONLY | O_CREAT | O_TRUNC, 0666),
           STDERR_FILENO) == -1) {
  ...
  // Write each name/count as a NameCountData struct to stdout (pipe write end)
  ENTRY *cur = nameList;
  while (cur) {
    NameCountData data;
    strncpy(data.name, cur->name, MAX_NAME_LENGTH);
    data.count = cur->count;
    write(STDOUT_FILENO, &data, sizeof(NameCountData));
    fprintf(fpout, "%s: %d\n", cur->name, cur->count);
    cur = cur->next;
  }
 ...
 * Assumption: nameList is a pointer to the head of the linked list containing
 * the names/counts. Name is null terminated and not empty. Input parameters:
 * name1 - the name to add to the list or update the count for Returns: none
 */
