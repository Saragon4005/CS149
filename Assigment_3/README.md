...
| Test Files Used | Expected Output | Edge Case Tested |
| --- | --- | --- |
| `test/names1.txt`, `test/names2.txt` | `% Jenn Xu: 2`, `Tom Wu: 4`, `%` | Tests aggregation of identical names across multiple files ("Tom Wu" appears 3x in names1 and 1x in names2, correctly aggregating to 4). Also tests empty line warnings printed to `.err` files. |
| `test/namesB.txt` | `% Nicky: 1`, `Dave Joe: 2`, `Yuan Cheng Chang: 3`, `John Smith: 1`, `%` | Tests accurate singular extraction and internal duplicate collapsing within a single file with diverse format blocks. |
| `test/names_long.txt` | `% Name: Count`, `%` | Tests file processing when individual lines exceed the 30-character boundary. Warnings are safely logged to `.err`. |
| `test/names1.txt`, `test/names2.txt`, `test/names1.txt`, `test/names_long.txt` | `% PATRICIA JOHNSON: 1`, `(98 other names)`, `Jenn Xu: 2`, `Tom Wu: 7`, `%` | Tests 4+ concurrent tasks using duplicate and heavy-load files simultaneously. Validates robustness under multiple parallel IPC writes. |
...
* Using `isatty` checking or distinct process redirection lets identical code function cohesively across both standard standalone shell execution and custom parent piping topologies.
